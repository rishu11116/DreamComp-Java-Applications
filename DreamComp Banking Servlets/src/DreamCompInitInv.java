...
public class DreamCompInitInv extends HttpServlet {
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response) throws ServletException,IOException {
		...
		String futAmountStr=request.getParameter("futAmount");
		String periodStr=request.getParameter("period");
		String rateStr=request.getParameter("rate");
		String termsStr=request.getParameter("terms");
		try {
			if(futAmountStr!=null && periodStr!=null && rateStr!=null && termsStr!=null) {
				futPrincipal=Double.parseDouble(futAmountStr);
		...
		"examples/servlet/DreamCompInitInv\">"+"<B>Enter the desired future value of the investment:</B>"+
				" <input type=textbox name=\"futAmount\""+" size=12 value=\"");
		pw.print(futAmountStr+"\">");
	...
	double compute() {
		...
		return futPrincipal/Math.pow(b,e);
	}
}